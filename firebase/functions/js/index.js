...
const isUsernameBlacklisted = async (namespace, username, reservedUsername) =>
...
const onRequest = (adminOnly, f) => async (req, res) => {
	try {
		if (req.get('X-Warmup-Ping')) {
			res.status(200).send('');
			return;
...
const notifyUser = async (data, namespace, username, serverInitiated) => {
