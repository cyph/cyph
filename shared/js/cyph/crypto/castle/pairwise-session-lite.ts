...
export class PairwiseSessionLite implements IPairwiseSession {
	...
	private readonly key		= (async () => {
		...
		if (this.handshakeState.isAlice) {
			...
			if ((await this.handshakeState.currentStep.getValue()) === HandshakeSteps.Start) {
				const [
					signingKeyPair,
					publicEncryptionKey
				]	= await Promise.all([
				...
				await this.handshakeState.initialSecretCyphertext.setValue(
					await this.potassium.box.seal(
						signingKeyPair ?
							await this.potassium.sign.sign(secret, signingKeyPair.privateKey) :
							secret
						,
						publicEncryptionKey
					)
	...
	constructor (
		...
		/** @ignore */
		protected readonly _INCOMING_MESSAGES: IAsyncValue<ICastleIncomingMessages> =
		...
		/** @ignore */
		protected readonly _RATCHET_STATE: IAsyncValue<ICastleRatchetState> =
