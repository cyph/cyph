...
testEnvironmentSetup = async (
	...
	const currentTestDataVersion = await databaseService
		.getItem('users/testDataVersion/certificate', BinaryProto)
		.catch(() => new Uint8Array(0));

	const localTestDataVersion = await localStorageService
		.getItem('testDataVersion', BinaryProto)
		.catch(() => undefined);
	...
	if (
		...
		(localTestDataVersion !== undefined &&
			potassiumUtil.compareMemory(
				currentTestDataVersion,
				localTestDataVersion
			))
	) {
		return;
	}
