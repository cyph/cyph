		...
		H = function (a)  {
			...
			for (var c = '', d = 0; ; ) {
				...
				64 != f &&
					...
					64 != h &&
						(c += String.fromCharCode(((f << 6) & 192) | h)));
			}
		};
	var I;
	function J (a, b)  {
			...
			return function (b)  {
				...
				a: {
					...
					for (var g = 0; g < e.length; g++) {
						...
						if (
							'_gl' ===
							decodeURIComponent(f[0]).replace(/\+/g, ' ')
						) {
							e = f.slice(1).join('=');
							break a;
						}
	...
	function Y (a, b)  {
		if (b && b.action) {
			...
			if ('get' === c) {
				...
				'post' === c &&
					((a = W(a, b.action)), q.test(a) && (b.action = a));
		}
	}
	var K = function (a)  {
			try {
...
(function ()  {
	...
	var p = /^(GTM|OPT)-[A-Z0-9]+$/,
		...
		t = function (a)  {
			...
			var c =
				'https://' + location.host + '/analytics/gtm/js?id=' + K(a.id);
		...
		if (!a.get('dcLoaded') && 'cookie' == a.get(ac)) {
			...
			if (se(a)) {
				...
				c ?
					...
					a.set(
						qe,
						{U: re(b, a, 1), google: re(b, a, 2), count: 0},
						!0
					)) :
					a.set(ad, 'b', !0);
			}
	...
	var u = function (a, b, c)  {
					...
					if (
						...
						if ((ca = ca.url))
							(a = (a =
								...
								null) ?
								a.nonce ||
								(a.getAttribute && a.getAttribute('nonce')) ||
								'' :
								''),
								e ?
									((e = ''),
		...
		B = function (a)  {
			...
			if (D(a.url, 'https://' + location.host + '/analytics/gtm/js?id='))
				return !0;
			if (
				a.query ||
				0 <= a.url.indexOf('?') ||
