...
const addRedoxCredentials	= async (isProd, url, username, redoxApiKey, redoxSecret) => {
...
for (let i = 0 ; i < 10 ; ++i) {
	...
	const apiKey		= await page.evaluate(() =>
		(document.querySelector('cyph-upload-ehr-credentials > div.done') || {}).textContent
	);
...
if (require.main === module) {
	(async () => {
		const isProd		= process.argv.slice(2).find(s => s === '--prod') !== undefined;
		...
		console.log(await addRedoxCredentials(isProd, url, username, redoxApiKey, redoxSecret));
		process.exit(0);
	})().catch(err => {
		console.error(err);
		process.exit(1);
